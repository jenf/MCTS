...
class MonteCarloSearch(object):
    ...
    def search(self,budget,root):
        ...
        for iter in range(budget):
            ...
            reward=self.calculateReward(front.state)
            self.updateNodes(front,reward)
    ...
    def TREEPOLICY(self,node):
        ...
        while node.state.terminal()==False:
            if node.fully_expanded()==False:    
                ...
                node=self.banditStrategy(node,SCALAR)
        return node
    ...
    def calculateReward(self, state):
        """Calculate the reward for the given state by finding the leaf"""
